...
def hIndex(citations: List[int]) -> int:
    ...
    for i in range(n_citations):
        # h_index is the first occurence in sorted citations
        # s.t there are h+ documents of h-citations
        if (n_citations - i) <= citations_sorted[i]:
            return n_citations - i
    return 0
